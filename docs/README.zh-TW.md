...
- **已解壓縮檔案資料夾**：提供從[步驟 1](#1-unpack-the-official-packages)中的 `UnpackedData` 資料夾的路徑。
  - 如果您是無條件合併，請跳過這一步。
- **語言包**：找到遊戲語言的檔案。
...
只需[解包](#1-unpack-the-official-packages)最新的更新檔案（最近的 `Patch*.pak`），然後再次[合併](#2-fill-the-fields-in-bg3-localization-merger)。
